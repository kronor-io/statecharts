...
data Env m = Env
  { envLogAction :: Colog.LogAction m Colog.Message,
    envPoolSize :: Natural,
    envPgSettings :: Hasql.Connection.Settings,
...
newEnv poolSize connectionSettings logger metrics = do
  pool <- makePgPool (fromIntegral poolSize) connectionSettings
  poolVar <- STM.newTVarIO pool
  return $ Env logger poolSize connectionSettings poolVar metrics
...
emergencyReplaceConnectionPool badPool = do
  env <- ask
  pool <- makePgPool (envPoolSize env) (envPgSettings env)
...
makePgPool ::
  ...
  Hasql.Connection.Settings ->
  m Hasql.Pool.Pool
makePgPool poolSize connectionSettings = liftIO do
  Hasql.Pool.acquire (fromIntegral poolSize, 30, connectionSettings)
