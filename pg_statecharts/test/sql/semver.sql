...
-- rejected versions
-- The inner block is what catches, so that the "should have been rejected"
-- raise cannot be swallowed by the handler meant for to_semver's own error.
do $$
...
begin
  ...
  if not rejected then
    raise exception '1.2.3.4 should have been rejected';
  end if;
...
begin
  ...
  if not rejected then
    raise exception 'banana should have been rejected';
  end if;
