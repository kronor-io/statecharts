...
one leaves the `fsm` tables empty and the identity sequences starting over from
1, so the next machines created collide with the ids the application tables
still carry, typically as a unique index violation on a `state_machine_id`
column. There is no repairing such a dump; take a fresh backup as soon as the
upgrade has been applied.

`pg_restore` loads the rows of `fsm.state` through the same trigger that fills
in `parent_path` on a normal insert, and that trigger needs the parent row to
already be there. `pg_dump` writes rows in the order they sit in the table,
which is insertion order unless a parent state was updated after its children
were created. If a restore ever fails on `fsm.state` with `Invalid parent_id`,
restore in sections and load the data with triggers off:

```bash
pg_restore -d mydb --section=pre-data backup.dump
pg_restore -d mydb --section=data --disable-triggers backup.dump
pg_restore -d mydb --section=post-data backup.dump
```

The registration only exists in the catalog of the database the extension was
created or upgraded in. A database that already had 0.1.0 installed from
before the registration was added has to `drop extension pg_statecharts
cascade` and create it again to pick it up, which also drops the fsm data, so
that only makes sense on a development database.
...
Both upgrades have to be run by a superuser, even though `superuser = false`
in the control file lets a plain user install the extension from scratch. The
update removes the catalogued dependency on the `semver` extension, and there
is no supported statement for dropping an extension-to-extension dependency
edge, so it deletes the `pg_depend` row directly; `drop extension semver`
below it needs to own that extension or be a superuser too.
...
`fsm.semver` domain, removes the now unnecessary dependency on `semver`,
drops the two file handling functions that 0.0.0 had and that have moved to
`pg_statecharts_dev`, and registers the fsm tables and sequences with
`pg_dump` (see [Backups](#backups)). Your data is preserved and migrations
generated by 0.0.0 keep deploying, reverting and verifying unchanged.
...
- **Your existing backups do not contain the machines.** Nothing registered the
  fsm tables with `pg_dump` before this, so every dump taken under 0.0.0 is
  missing them. Take a new backup right after upgrading.
...
### Installing `semver` afterwards

Only one of the two orders is handled. Installing pg_statecharts into a
database that already has `semver` works, because the alias steps aside. The
other way round it does not: pg_statecharts took the name first, and
`create extension semver` fails with

```
ERROR:  cannot change return type of existing function
HINT:  Use DROP FUNCTION to_semver(text) first.
```

The hint is not enough on its own, because the alias belongs to the extension
and `drop function` refuses for that reason. Release it first:
...
Nothing in pg_statecharts calls the alias, so dropping it costs you only the
migrations generated by 0.0.0, which need regenerating against 0.1.0 anyway.
Both orders are asserted in `.github/scripts/check_semver_coexistence.sh`.
