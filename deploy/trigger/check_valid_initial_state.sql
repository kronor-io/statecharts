...
BEGIN;
  ...
  create or replace function fsm.trig_check_valid_initial_state()
  returns trigger as
  $$
  declare total_initial int;
  begin
  ...
  drop trigger if exists check_valid_initial_state_insert on fsm.compound_state;
  drop trigger if exists check_valid_initial_state_update on fsm.compound_state;
  ...
  deferrable initially deferred
  for each row
  when (NEW.is_initial <> OLD.is_initial or NEW.statechart_id <> OLD.statechart_id)
  execute function fsm.trig_check_valid_initial_state();
